...
function makeMemoryStore() {
  ...
  return {
    ...
    async loginUser(payload) {
      ...
      const city = user.role === 'guide' && payload.city !== undefined ? normalizeCity(payload.city) : user.city;
      if (displayName || city !== user.city) { user.name = displayName || user.name; user.city = city; user.updatedAt = now(); }
      const token = await createAuthSession(user);
      return { user: publicUser(user), token };
    },
    ...
    async acceptRequest(requestId, user) {
      ...
      if (request.status === 'cancelled') throw requestCancelledConflict();
      if (request.status === 'accepted' && request.sessionId) return { request: publicRequest(request), session: publicSession(memory.liveSessions.get(request.sessionId)) };
    ...
    async cancelRequest(requestId, user) {
      const request = memory.requests.get(requestId);
      if (!request || user.role !== 'traveler' || request.travelerId !== user.id) return null;
      if (request.status === 'cancelled') return this.getRequest(requestId, user);
      if (request.status !== 'pending' && request.status !== 'accepted') throw cancelConflict();
      const updatedAt = now();
      request.status = 'cancelled'; request.updatedAt = updatedAt;
      if (request.sessionId) {
        const session = memory.liveSessions.get(request.sessionId);
        if (session) {
          session.status = 'cancelled'; session.updatedAt = updatedAt;
          await this.addMessage(session.id, { text: 'Traveler cancelled this walk.' }, { ...user, role: 'system', name: 'LiveWalk' }, true);
        }
      }
    ...
    async getSession(sessionId, user) {
      ...
      return { session: publicSession(session), request: publicRequestForUser(request, user), messages: memory.messages.get(sessionId) ?? [] };
    },
    ...
    async addMessage(sessionId, payload, user, system = false) {
      ...
      if (!system && (request.status === 'cancelled' || session.status === 'cancelled')) throw requestCancelledConflict();
    ...
    async mintLiveKitToken(sessionId, user, config) {
      const session = memory.liveSessions.get(sessionId);
      const request = session ? memory.requests.get(session.requestId) : null;
...
function makeDbStore(env, useDb = (operation) => withDb(env, operation)) {
  async function readRequest(client, requestId, user) {
    const req = rowToRequest((await client.query(requestSelect('WHERE r.id=$1'), [requestId])).rows[0]);
    if (!canReadRequest(user, req)) return null;
    let session = null;
    if (req.sessionId) session = rowToSession((await client.query('SELECT * FROM livewalk_sessions WHERE id=$1', [req.sessionId])).rows[0]);
    return { request: publicRequestForUser(req, user), session: publicSession(session) };
  }

  async function readSession(client, sessionId, user) {
    const session = rowToSession((await client.query('SELECT * FROM livewalk_sessions WHERE id=$1', [sessionId])).rows[0]);
    const req = session ? rowToRequest((await client.query(requestSelect('WHERE r.id=$1'), [session.requestId])).rows[0]) : null;
    if (!session || !canUseSession(user, req)) return null;
    const messages = await client.query('SELECT * FROM livewalk_messages WHERE session_id=$1 ORDER BY created_at ASC', [sessionId]);
    return { session: publicSession(session), request: publicRequestForUser(req, user), messages: messages.rows.map(rowToMessage) };
  }
  ...
  return {
    ...
    async loginUser(payload) { return withDb(env, async (client) => { const result = await client.query('SELECT * FROM livewalk_users WHERE email=$1', [normalizeEmail(payload.email)]); const user = rowToUser(result.rows[0]); if (!user || !(await verifyPassword(String(payload.password || ''), user.passwordSalt, user.passwordHash))) throw new Error('Invalid email or password'); const displayName = String(payload.name || payload.displayName || '').trim(); const city = user.role === 'guide' && payload.city !== undefined ? normalizeCity(payload.city) : user.city; if ((displayName && displayName !== user.name) || city !== user.city) { const updatedAt = now(); await client.query('UPDATE livewalk_users SET name=$1, city=$2, updated_at=$3 WHERE id=$4', [displayName || user.name, city, updatedAt, user.id]); user.name = displayName || user.name; user.city = city; user.updatedAt = updatedAt; } const token = randomBase64(32); await client.query('INSERT INTO livewalk_auth_sessions (token_hash,user_id,expires_at,created_at) VALUES ($1,$2,$3,$4)', [await sha256(token), user.id, new Date(Date.now() + TOKEN_DAYS * 86400000).toISOString(), now()]); return { user: publicUser(user), token }; }); },
    ...
    async getRequest(requestId, user) { return useDb((client) => readRequest(client, requestId, user)); },
    async acceptRequest(requestId, user) { if (user.role !== 'guide') return null; return useDb(async (client) => { await client.query('BEGIN'); try { const req = rowToRequest((await client.query('SELECT * FROM livewalk_requests WHERE id=$1 FOR UPDATE', [requestId])).rows[0]); if (!req || !canReadRequest(user, req) || (req.guideId && req.guideId !== user.id)) { await client.query('ROLLBACK'); return null; } if (req.status === 'cancelled') throw requestCancelledConflict(); if (req.status === 'accepted' && req.sessionId) { await client.query('COMMIT'); return readRequest(client, requestId, user); } if (req.status !== 'pending') { await client.query('ROLLBACK'); return null; } const guide = makeGuide(user); const sessionId = req.sessionId || id('sess'); const updatedAt = now(); await client.query('UPDATE livewalk_requests SET status=$1, guide_id=$2, guide=$3::jsonb, session_id=$4, updated_at=$5 WHERE id=$6', ['accepted', user.id, JSON.stringify(guide), sessionId, updatedAt, requestId]); await client.query(`INSERT INTO livewalk_sessions (id, request_id, status, started_at, location, created_at, updated_at) VALUES ($1,$2,'ready',NULL,NULL,$3,$3) ON CONFLICT (id) DO NOTHING`, [sessionId, requestId, updatedAt]); const msgId = id('msg'); await client.query('INSERT INTO livewalk_messages (id, session_id, sender_role, sender_name, text, created_at) VALUES ($1,$2,$3,$4,$5,$6)', [msgId, sessionId, 'system', 'LiveWalk', `${guide.name} accepted the walk.`, now()]); await client.query('COMMIT'); return readRequest(client, requestId, user); } catch (e) { await client.query('ROLLBACK').catch(() => {}); throw e; } }); },
    async cancelRequest(requestId, user) { if (user.role !== 'traveler') return null; return useDb(async (client) => { await client.query('BEGIN'); try { const req = rowToRequest((await client.query('SELECT * FROM livewalk_requests WHERE id=$1 FOR UPDATE', [requestId])).rows[0]); if (!req || req.travelerId !== user.id) { await client.query('ROLLBACK'); return null; } if (req.status === 'cancelled') { await client.query('COMMIT'); return readRequest(client, requestId, user); } if (req.status !== 'pending' && req.status !== 'accepted') throw cancelConflict(); const updatedAt = now(); await client.query('UPDATE livewalk_requests SET status=$1, updated_at=$2 WHERE id=$3', ['cancelled', updatedAt, requestId]); if (req.sessionId) { await client.query('UPDATE livewalk_sessions SET status=$1, updated_at=$2 WHERE id=$3', ['cancelled', updatedAt, req.sessionId]); await client.query('INSERT INTO livewalk_messages (id, session_id, sender_role, sender_name, text, created_at) VALUES ($1,$2,$3,$4,$5,$6)', [id('msg'), req.sessionId, 'system', 'LiveWalk', 'Traveler cancelled this walk.', updatedAt]); } await client.query('COMMIT'); return readRequest(client, requestId, user); } catch (error) { await client.query('ROLLBACK').catch(() => {}); throw error; } }); },
    async declineRequest(requestId, user) { if (user.role !== 'guide') return null; return withDb(env, async (client) => { await client.query('BEGIN'); try { const req = rowToRequest((await client.query('SELECT * FROM livewalk_requests WHERE id=$1 FOR UPDATE', [requestId])).rows[0]); if (!req) { await client.query('ROLLBACK'); return null; } if (req.status === 'cancelled') throw requestCancelledConflict(); if (req.status !== 'pending' || req.guideId) throw declineConflict(); await client.query('INSERT INTO livewalk_declines (request_id, guide_id, created_at) VALUES ($1,$2,$3) ON CONFLICT (request_id, guide_id) DO NOTHING', [requestId, user.id, now()]); await client.query('COMMIT'); return publicRequest(req); } catch (error) { await client.query('ROLLBACK').catch(() => {}); throw error; } }); },
    async getSession(sessionId, user) { return useDb((client) => readSession(client, sessionId, user)); },
    async startSession(sessionId, user) { return useDb(async (client) => { const session = rowToSession((await client.query('SELECT * FROM livewalk_sessions WHERE id=$1', [sessionId])).rows[0]); const req = session ? rowToRequest((await client.query(requestSelect('WHERE r.id=$1'), [session.requestId])).rows[0]) : null; if (!session || !canUseSession(user, req) || user.role !== 'guide' || req.guideId !== user.id) return null; if (req.status === 'cancelled' || session.status === 'cancelled') throw requestCancelledConflict(); const updatedAt = now(); await client.query('UPDATE livewalk_sessions SET status=$1, started_at=COALESCE(started_at,$2), updated_at=$2 WHERE id=$3', ['live', updatedAt, sessionId]); await client.query('UPDATE livewalk_requests SET status=$1, updated_at=$2 WHERE id=$3', ['live', updatedAt, session.requestId]); await client.query('INSERT INTO livewalk_messages (id, session_id, sender_role, sender_name, text, created_at) VALUES ($1,$2,$3,$4,$5,$6)', [id('msg'), sessionId, 'system', 'LiveWalk', 'The live walk session started.', now()]); return readSession(client, sessionId, user); }); },
    async endSession(sessionId, user) { return useDb(async (client) => { await client.query('BEGIN'); try { const session = rowToSession((await client.query('SELECT * FROM livewalk_sessions WHERE id=$1 FOR UPDATE', [sessionId])).rows[0]); const req = session ? rowToRequest((await client.query('SELECT * FROM livewalk_requests WHERE id=$1 FOR UPDATE', [session.requestId])).rows[0]) : null; if (!session || !canUseSession(user, req)) { await client.query('ROLLBACK'); return null; } if (req.status === 'cancelled' || session.status === 'cancelled') throw requestCancelledConflict(); if (session.status === 'ended') { await client.query('COMMIT'); return readSession(client, sessionId, user); } const endedAt = now(); await client.query('UPDATE livewalk_sessions SET status=$1, ended_at=COALESCE(ended_at,$2), updated_at=$2 WHERE id=$3', ['ended', endedAt, sessionId]); await client.query('UPDATE livewalk_requests SET status=$1, updated_at=$2 WHERE id=$3', ['completed', endedAt, session.requestId]); await client.query('INSERT INTO livewalk_messages (id, session_id, sender_role, sender_name, text, created_at) VALUES ($1,$2,$3,$4,$5,$6)', [id('msg'), sessionId, 'system', 'LiveWalk', 'The live walk session ended.', endedAt]); await client.query('COMMIT'); return readSession(client, sessionId, user); } catch (error) { await client.query('ROLLBACK').catch(() => {}); throw error; } }); },
...
export { store, requireUser, rowToRequest, makeDbStore };
